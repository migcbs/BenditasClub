...
const CartPopup = ({ cartItems, onClose, removeFromCart, clearCart }) => {
  ...
  const resetForm = () => {
    ...
    setDireccion(''); setSucursal(''); setErrores({});
  };
  ...
  const generarMensaje = (numOrden) => {
    ...
    cartItems.forEach((item, i) => {
      ...
      if (variante && variante !== 'Único' && variante !== 'Default') msg += ` (${variante})`;
    ...
    <div className="cart-popup-overlay" onClick={onClose}>
      <div className="cart-popup" onClick={e => e.stopPropagation()}>
        ...
        <div className="cart-popup-header">
          <h2>{showForm ? 'Datos del pedido' : 'Tu carrito'}</h2>
          <button className="cart-popup-close" onClick={onClose}>×</button>
        </div>

        {cartItems.length === 0 && (
          <p className="cart-popup-empty">Tu carrito está vacío — ¡agrega algo del merch!</p>
        )}
          ...
          <>
            <div className="cart-popup-items">
              {cartItems.map(item => (
                <div key={`${item.id}-${item.variantId ?? item.varianteId}`} className="cart-item">
                  <img src={item.image ?? item.imagen} alt={getNombre(item)} className="cart-item-image" />
                  ...
                  <p className="cart-item-price">${(getPrice(item) * getCantidad(item)).toFixed(2)}</p>
                  <button
            ...
            <div className="cart-popup-footer">
              ...
              <div className="cart-popup-actions">
                <button className="cart-btn-clear" onClick={confirmarVaciar}>Vaciar</button>
        ...
        {cartItems.length > 0 && showForm && (
          <div className="cart-form">
            ...
            <div className="cart-form-group">
              <input type="tel" placeholder="Teléfono (10 dígitos)" value={telefono}
                maxLength={10} onChange={e => setTelefono(e.target.value)}
            ...
            <div className="cart-form-group">
              ...
              <div className="cart-metodo-btns">
                <button type="button"
                  ...
                  onClick={() => setMetodo('delivery')}>🛵 A domicilio</button>
                <button type="button"
                  className={`cart-metodo-btn ${metodo === 'pickup' ? 'activo' : ''}`}
                  onClick={() => { setMetodo('pickup'); setDireccion(''); }}>🏪 Recoger</button>
              </div>
              {errores.metodo && <span className="cart-form-error">{errores.metodo}</span>}
            </div>
            {metodo === 'delivery' && (
              <div className="cart-form-group">
                <input type="text" placeholder="Calle, número, colonia..." value={direccion}
              ...
              <div className="cart-metodo-btns">
                {Object.keys(WHATSAPP_NUMEROS).map(s => (
                  <button key={s} type="button"
                    ...
                    onClick={() => setSucursal(s)}>📍 {s}</button>
                ))}
              </div>
            ...
            <div className="cart-popup-footer">
              <div className="cart-popup-actions">
                <button className="cart-btn-clear" onClick={() => setShowForm(false)}>Atrás</button>
