...
export const adminApi = {
  ...
  coupons: (token) => fetch(`${API_BASE}/api/admin/coupons`, { headers: auth(token) }).then(handle),
  createCoupon: (payload, token) => fetch(`${API_BASE}/api/admin/coupons`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...auth(token) }, body: JSON.stringify(payload) }).then(handle),
  updateCoupon: (id, payload, token) => fetch(`${API_BASE}/api/admin/coupons/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', ...auth(token) }, body: JSON.stringify(payload) }).then(handle),
  deleteCoupon: (id, token) => fetch(`${API_BASE}/api/admin/coupons/${id}`, { method: 'DELETE', headers: auth(token) }).then((res) => { if (!res.ok) throw new Error('No se pudo eliminar el cupón'); }),
